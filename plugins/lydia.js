...
Amdi.applyCMD({pattern: 'addlydia$', fromMe: true,  deleteCommand: false,  desc: Lang.ADDLYDIA_DESC}, (async (message, match) => {
...
Amdi.applyCMD({pattern: 'rmlydia$', fromMe: true,  deleteCommand: false,  desc: Lang.RMLYDIA_DESC}, (async (message, match) => {
    if (!message.reply_message) return await message.reply(Lang.NEED_REPLY);
    if (!Config.COFFEEHOUSE_API_KEY) return await message.reply(Lang.COFFEEHOUSE);
    var unix = Date.now() / 1000 | 0;
