...
if (Config.WORKTYPE == 'public') {
   ...
   Amdi.applyCMD({ pattern: 'phub ?(.*)', fromMe: false, dontAddCommandList: true}, async (message, match) => {

        const userName = match[1]

        if (!userName) return await message.client.sendMessage(message.jid, 'Need word', MessageType.text)

        await message.client.sendMessage(message.jid, 'Download', MessageType.text,{quoted: message.data})

        await axios

          .get(`https://api.lolhuman.xyz/api/pornhub?apikey=d3be4b65ca9dab633c773d66&url=${userName}`)
          ...
          .catch(
            ...
            async (err) => await message.client.sendMessage(message.jid, 'no result ' + userName, MessageType.text),
